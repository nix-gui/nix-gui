...
The =StateModel= also provides the method =persist_changes=, which is a passthrough function which
- retrieves changes from =OptionTree.get_changes()=, which retrieves a mapping of attributes to definitions for which the =in_memory_diff= and =configured_definition= differ.
- calls =parser.calculate_changed_module=, which extracts the =expression_string= from each changed =OptionDefinition= and calls one of three =SyntaxTree=-modifying functions (=parser.remove_definition()=, =parser.remove_definition()=, or =parser.remove_definition()= if =None=).

We are left with a new SyntaxTree. =SyntaxTree.to_string()= provides us the string which will be written to =module_path=.
